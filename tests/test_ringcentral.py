...
class TestRingCentralClientOAuth:
    ...
    def test_adaptive_card_methods_use_verified_endpoints(self):
        ...
        created = asyncio.run(
            client.create_adaptive_card(
                "g-1",
                {"type": "Wrong", "version": "1.3", "body": []},
            )
        )
        read = asyncio.run(client.get_adaptive_card("card-1"))
        updated = asyncio.run(
            client.update_adaptive_card(
                "card-1",
                {"version": "1.3", "body": []},
            )
        )
        ...
        assert calls == [
            (
                ...
                {"json_body": {"type": "AdaptiveCard", "version": "1.3", "body": []}},
            ),
            ("GET", "/team-messaging/v1/adaptive-cards/card-1", {}),
            (
                "PUT",
                "/team-messaging/v1/adaptive-cards/card-1",
                {"json_body": {"version": "1.3", "body": [], "type": "AdaptiveCard"}},
            ),
            (
                "DELETE",
                "/team-messaging/v1/adaptive-cards/card-1",
...
class TestRingCentralHistoryTool:
    ...
    @staticmethod
    def _call_artifact_tool(handler, args: dict, *, platform: str = "ringcentral", chat_id: str = "g-1") -> dict:
        from gateway.session_context import set_session_vars
    ...
    def test_adaptive_card_tools_use_current_ringcentral_session(self, monkeypatch):
        monkeypatch.setenv("RC_BOT_TOKEN", "bot-token")
        client = MagicMock()
        client.create_adaptive_card = AsyncMock(return_value={"id": "card-1", "type": "AdaptiveCard"})
        client.get_adaptive_card = AsyncMock(return_value={"id": "card-1", "type": "AdaptiveCard"})
        client.update_adaptive_card = AsyncMock(return_value={"id": "card-1", "type": "AdaptiveCard"})
        client.delete_adaptive_card = AsyncMock(return_value=True)
        client.close = AsyncMock()

        with patch.object(_rc_mod, "RingCentralClient", return_value=client):
            created = self._call_artifact_tool(
                _ringcentral_create_adaptive_card,
                {"text": "hello card"},
                chat_id="g-1",
            )
            read = self._call_artifact_tool(
                _ringcentral_get_adaptive_card,
                {"card_id": "card-1"},
                chat_id="g-1",
            )
            updated = self._call_artifact_tool(
                _ringcentral_update_adaptive_card,
                {"card_id": "card-1", "card": {"type": "Wrong", "body": []}},
                chat_id="g-1",
            )
            deleted = self._call_artifact_tool(
                _ringcentral_delete_adaptive_card,
                {"card_id": "card-1"},
                chat_id="g-1",
            )
        ...
        client.create_adaptive_card.assert_awaited_once_with(
            "g-1",
            {
                "version": "1.3",
                "body": [{"type": "TextBlock", "text": "hello card", "wrap": True}],
                "type": "AdaptiveCard",
            },
        )
        client.update_adaptive_card.assert_awaited_once_with(
            "card-1",
